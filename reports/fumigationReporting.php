...
if($_POST) {
	...
	$sql = "select fumigation.supervisionRef, fumigation.station, fumigation.coach_no, fumigation.approvalPM, fumigation.approvalPC, fumigation.Supervisor, fumigation.date, supervisors.fname, supervisors.sname
			FROM fumigation
			INNER JOIN supervisors ON fumigation.Supervisor=supervisors.supervisor_id WHERE fumigation.dateFilter >= '$start_date' AND fumigation.dateFilter <= '$end_date' ORDER BY fumigation.date ASC";
