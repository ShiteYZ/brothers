...
if($_POST) {
	...
	$startDate = $_POST['startDateMiradi'];
	$date = DateTime::createFromFormat('m/d/Y',$startDate);
	$start_date = $date->format("Y-m-d");


	$endDate = $_POST['endDateMiradi'];
	...
	$sql1="select wekezomiradi.wekezo_id, wekezomiradi.mradi_id, wekezomiradi.thamani, wekezomiradi.lengo, miradi.jina
			FROM wekezomiradi
			INNER JOIN miradi ON wekezomiradi.mradi_id=miradi.id WHERE wekezomiradi.tarehe >= '$start_date' AND wekezomiradi.tarehe <= '$end_date' ORDER BY wekezomiradi.tarehe ASC";
